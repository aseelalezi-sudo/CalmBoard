      ...
      {columns.map((col) => {
        ...
        return (
          ...
          >
            ...
            {col.id === "assignee" && (
              <TaskAssignmentControl task={subtask} ctx={ctx} size={16} maxVisible={2} showLabel={false} />
            )}
...
export function AdvancedTaskTable({ ctx }: { ctx: ViewCtx }) {
  ...
  const columns = useMemo<ColumnDef<Task>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TaskAssignmentControl task={row.original} ctx={ctx} size={20} maxVisible={3} showLabel={false} />
        ),
      },
      {
        id: "points",
  ...
  const bulkPriorityChange = async (priority: string) => {
    const selectedTasks = ctx.tasks.filter((t) => selectedIds.includes(t.id));
    const results = await Promise.allSettled(
      selectedTasks.map((t) => ctx.updateTask(t.id, { expectedVersion: t.version, priority })),
    );
    const failures = results.filter((r) => r.status === "rejected" || (r.status === "fulfilled" && r.value === false));
    if (failures.length > 0) {
      ctx.notify(
        ...
        {selectedIds.length > 0 &&
          ...
          createPortal(
            <div className="fixed bottom-7 inset-x-0 mx-auto w-fit max-w-[calc(100%-1rem)] z-9999 flex items-center gap-2.5 rounded-2xl border border-line/90 bg-surface/95 dark:bg-zinc-900/95 px-4 py-2.5 shadow-[0_20px_50px_rgba(0,0,0,0.35)] backdrop-blur-xl animate-slide-up text-ink ring-1 ring-black/10 dark:ring-white/10 select-none">
              ...
              <select
                name="bulk-remove-assignee-select"
                defaultValue=""
                onChange={(event) => {
