...
export function AIPanel({
  ...
  return (
    <div className="fixed inset-0 z-[65] flex justify-end">
      ...
      />
      <div
        className="theme-adaptive-panel animate-slide relative flex w-full max-w-[430px] flex-col border-s border-slate-200 bg-white/98 text-slate-900 shadow-2xl dark:border-white/[0.08] dark:bg-[#0d0d15]/98 dark:text-zinc-100"
        style={{ "--slide-x": "-32px" } as CSSProperties}
      >
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-indigo-100 bg-gradient-to-r from-indigo-500/10 to-violet-500/10 px-5 dark:border-white/[0.06] dark:from-indigo-500/15 dark:to-violet-400/10">
          <div className="flex items-center gap-3">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 text-white shadow-[0_0_16px_rgba(99,102,241,0.4)]">
              <IconSparkle size={16} />
            </span>
            <div>
          ...
          <button
            ...
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 hover:bg-indigo-50 hover:text-indigo-700 dark:text-zinc-500 dark:hover:bg-white/[0.06] dark:hover:text-white"
          >
            <IconX size={15} />
          </button>
        </header>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-5">
          <div className="rounded-xl border border-violet-200 bg-violet-50/70 p-3.5 dark:border-violet-500/20 dark:bg-violet-500/[0.06]">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={t(
                "مثال: خطة إطلاق منتج تشمل التصميم والتطوير والتسويق",
                "e.g. Launch plan covering design, development, and marketing",
              )}
              className={cn(areaCls, "mt-2")}
            />
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
              <Btn variant="primary" disabled={loading} onClick={() => run("breakdown")}>
                <IconLayers size={13} />
                {t("قسّم المهمة", "Breakdown")}
              </Btn>
              <Btn variant="outline" disabled={loading} onClick={() => run("generate_task")}>
                <IconPlus size={13} />
                {t("اقترح مهمة", "Suggest task")}
              </Btn>
              <Btn variant="outline" disabled={loading} onClick={() => run("summarize")}>
                <IconDoc size={13} />
                {t("لخّص المشروع", "Summarize")}
              </Btn>
              <Btn variant="outline" disabled={loading} onClick={() => run("priority")}>
                <IconBolt size={13} />
                {t("اقترح أولوية", "Priority")}
              </Btn>
              <Btn variant="outline" disabled={loading} onClick={() => run("report")}>
                <IconDoc size={13} />
          ...
          {proposal && (
            <section className="rounded-xl border border-amber-300 bg-amber-50 p-4 dark:border-amber-400/25 dark:bg-amber-400/[0.06]">
              ...
              <div className="mt-3 max-h-56 space-y-2 overflow-y-auto">
                {proposal.tasks.map((task, index) => (
                  <div
                    ...
                    className="rounded-lg border border-amber-200 bg-white p-2.5 dark:border-white/[0.08] dark:bg-white/[0.04]"
                  >
        ...
        <footer className="border-t border-slate-200 p-4 text-[10.5px] text-slate-500 dark:border-white/[0.06] dark:text-zinc-600">
