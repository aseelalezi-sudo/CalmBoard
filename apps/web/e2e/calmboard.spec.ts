...
async function registerIsolatedOwner(page: Page) {
  ...
  const languageToggle = page.getByTitle(arabic.toggleLanguage);
  await expect(languageToggle).toBeVisible();
  await languageToggle.click();
  await expect(page.locator("html")).toHaveAttribute("dir", "ltr");
  await expect(page.getByText(identity.workspace, { exact: true })).toBeVisible();
  return identity;
}
...
test.describe("CalmBoard core acceptance", () => {
  test("owner creates a project, task, subtask, and comment through the real UI", async ({ page }) => {
    ...
    await projectsLabel.locator("..").getByRole("button").click();
